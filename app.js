...
// ========== YTS FREE DOWNLOAD (NEW) ==========
async function getYTSDownloadLink(title) {
  try {
    ...
    if (data.data && data.data.movies && data.data.movies.length > 0) {
      ...
      if (torrents && torrents.length > 0) {
        const t1080 = torrents.find(t => t.quality === '1080p');
        const t720 = torrents.find(t => t.quality === '720p');
        const best = t1080 || t720 || torrents[0];
...
window.handleYTSDownload = async function(movieId) {
  ...
  try {
    const torrentUrl = await getYTSDownloadLink(movie.title);
    if (torrentUrl) {
      window.open(torrentUrl, '_blank');
      alert('✅ Download started! (Torrent file)');
    } else {
      alert('❌ No torrent found. Try Server 1.');
    }
  } catch (error) {
    console.error('Download error:', error);
    alert('❌ Error fetching link. Please try again.');
  } finally {
    if (btn) {
      btn.innerHTML = 'SERVER 2 (FREE)';
      btn.disabled = false;
    }
  }
};
// ========== END YTS FREE DOWNLOAD ==========
      ...
      <div class="detail-content">
        ...
        <div class="download-section" style="background: #161d2f; border: 1px solid #232d45; border-radius: 12px; padding: 20px; margin-top: 30px;">
          ...
          <div style="display: flex; gap: 12px; width: 100%;">
            ...
            <!-- Server 2: YTS (NEW) -->
            <button id="yts-download-btn" onclick="handleYTSDownload(${movie.id})" style="flex: 1; display: flex; flex-direction: column; align-items: center; justify-content: center; background: #232d45; color: #fff; padding: 12px; border-radius: 8px; border: none; cursor: pointer; transition: background 0.2s;">
              <div style="font-size: 1rem; font-weight: 800;">SERVER 2 (FREE)</div>
              <div style="font-size: 0.75rem; opacity: 0.85; margin-top: 4px;">YTS TORRENT</div>
            </button>
          </div>
        </div>
        `}
